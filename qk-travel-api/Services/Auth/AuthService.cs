...
namespace QkTravelApi.Services.Auth
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<LoginResponse> LoginAsync(LoginRequest loginDto)
        {
            ...
            // Check if email is confirmed (TẠM TẮT ĐỂ DEMO)
        ...
        private async Task<LoginResponse> GenerateAuthResponse(ApplicationUser user)
        {
            ...
            var planName = await _userPlanSubscriptionService.GetCurrentPlanNameAsync(user.Id);
            userResponse.SubscriptionPlan = planName;
