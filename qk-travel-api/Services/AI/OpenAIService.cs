...
- Ưu tiên sử dụng dữ liệu thật trong mục CRAWLED_TRAVEL_DATA nếu có
- Không tự bịa tên khách sạn/quán ăn/địa điểm khi dữ liệu crawl đã cung cấp lựa chọn phù hợp
...
GIÁ TIỀN (BẮT BUỘC):
- Mọi giá (pricePerNight, priceFrom, priceTo, totalEstimatedCostFrom, totalEstimatedCostTo) luôn > 0, không bao giờ để 0.
- Nếu CRAWLED_TRAVEL_DATA có price thì dùng con số đó; nếu không, ƯỚC LƯỢNG hợp lý theo ngân sách của khách.
- totalEstimatedCost là tổng chi phí cả chuyến cho tất cả người, ước lượng theo số ngày và số người.";
...
- Ưu tiên tên địa điểm, ảnh, nguồn và mô tả từ CRAWLED_TRAVEL_DATA
- Nếu dùng item crawl có imageUrl thì giữ lại imageUrl đó, không thay bằng URL khác

GIÁ TIỀN (BẮT BUỘC):
- priceFrom và priceTo của MÓN ĂN luôn > 0 (đơn vị VNĐ). Món rẻ nhất cũng từ 20,000-50,000. Không bao giờ để 0.
- Nếu CRAWLED_TRAVEL_DATA có price thì dùng con số đó; nếu không, ƯỚC LƯỢNG hợp lý theo ngân sách {context.PriceRange}.
- Hoạt động miễn phí (đi dạo, ngắm cảnh) thì priceFrom=0 và priceTo=0; hoạt động có vé thì điền giá thật > 0.
...
## CRAWLED_TRAVEL_DATA
{BuildCrawledTravelDataBlock(context)}

## OUTPUT FORMAT
{{
  ""summary"": ""Tóm tắt hấp dẫn về chuyến đi (2-3 câu)"",
  ""totalEstimatedCostFrom"": 3000000,
  ""totalEstimatedCostTo"": 5000000,
  ""accommodations"": [
    {{
...
{{
  ...
  ""activities"": [
    {{
      ...
      ""priceFrom"": 0,
      ""priceTo"": 0,
      ""tips"": ""Tips cụ thể từ người bản địa"",      ""mapUrl"": ""https://www.google.com/maps/search/..."",
      ""imageUrl"": ""https://tse1.mm.bing.net/th?q=..."",
      ""order"": 1
    }}
  ],
...
BẮT BUỘC: 3 activities và ít nhất 2-3 món ăn. Viết chi tiết, cảm xúc như blog du lịch. Chỉ trả JSON thuần túy.";
        ...
        private string BuildCrawledTravelDataBlock(AIGenerationContext context)
        {
            if (context.CrawledTravelItems == null || !context.CrawledTravelItems.Any())
            {
                return "Chưa có dữ liệu crawl đã duyệt cho địa điểm này. Có thể dùng kiến thức du lịch chung nhưng không được bịa chi tiết quá cụ thể như số điện thoại hoặc giá chính xác.";
            }

            var items = context.CrawledTravelItems.Take(30).Select((item, index) =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(item.Address)) fields.Add($"address={item.Address}");
                if (!string.IsNullOrWhiteSpace(item.ImageUrl)) fields.Add($"imageUrl={item.ImageUrl}");
                if (item.Rating.HasValue) fields.Add($"rating={item.Rating}");
                if (!string.IsNullOrWhiteSpace(item.PriceText)) fields.Add($"price={item.PriceText}");
